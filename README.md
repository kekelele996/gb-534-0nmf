...
- 偏差分析：冻结配方与时序输入，执行阶段约束 DTW，展示阶段证据、对齐曲线和疑似原因；缺失率超过 20% 的通道自动隔离并按剩余通道降权复算，保留隔离前后阶段分与权重变化。
...
通道隔离与降权复算（`phase-dtw-v1.1.0`）：某通道全局缺失率严格超过 20% 时，该通道会被整次隔离，不再拖垮评分；系统按剩余有效通道重新计算各阶段权重，并在结果中同时保留隔离前后阶段分、阶段权重、隔离通道缺失率与降权比例，页面展示受影响阶段、隔离通道和降权比例。出现以下任一情况时整次分析被拒绝（返回 422，不产生结果，原时序保持不变）：隔离后有效通道少于 2 个、任一阶段没有可用观测点、输入时序未就绪。隔离判定只依赖冻结输入，相同输入重放必然得到同一隔离结果。
