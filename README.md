...
自 `phase-dtw-v1.1.0` 起，分析包含通道隔离与降权复算：任一参考通道在配方时间窗内缺失率超过 20% 时，该通道被标为隔离，阶段权重视其贡献降为 0 并按剩余有效通道重算阶段分与总分；结果保留隔离前后阶段分、阶段权重、通道权重和总分，并在分析页与解释抽屉展示受影响阶段、隔离通道、缺失率和降权比例。隔离后有效通道少于 2 个、任一阶段没有可用点，或时序未就绪/输入未解析时，整次分析直接拒绝（分析置为 `failed`，接口返回 422），不产生新结果。隔离判定只依赖冻结输入，相同输入重放必然得到同一隔离结论；旧版 `phase-dtw-v1.0.0` 快照仍可只读重放以核对历史结果。
